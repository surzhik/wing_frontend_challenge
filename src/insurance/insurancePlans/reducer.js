...
export const getValue = (state, value) =>
  getResources(state, plan => isForValue(plan, value));
export const get = (state, id) => state.resources[id];
export const getAll = (state, sequence) =>
  getResources(state).reduce((plans, plan) => {
    const planIndex = sequence.indexOf(plan.sku);
    if (planIndex >= 0) {
