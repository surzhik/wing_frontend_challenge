...
const SelectedPlan = props => {
  const { insurancePlan: plan, onConfirm, activating } = props;
      ...
      <Box>
        <div className={styles.wrap}>
          ...
          <div className={styles.plan}>
            <h4 className={styles.header}>You Selected</h4>
            <div className={styles.name}>{plan.name}</div>
            <div className={styles.info}>{planInfo(plan.sku)}</div>
          </div>
        </div>
      </Box>
      <Box className={styles.doConfirm}>
        <PlanInfo insurancePlan={plan} />
        <BottomButton onClick={onConfirm} disabled={activating}>
